...
if (strlen($btnBckup) <> 0)
{
	// Include your user and password $user $pass
	// outside the public folder
	//include("/home/sitename/connect/user.php"); 
	// Mysql Database Information:
		$database = 'hrmis';   // name of the database.
		$username = 'hrmisadmin';  // username with access to database.
		$password = 'hradmin';  // password for username.
	// Database Backup Filename & Location
		$backupto = '/var/tmp/hrmis';  // absolute path to folder containing database - no trailing slash.
		$current_date = date("Y-m-d-Gis");
		$backupas = $database .$current_date. '.tar' . '.gz';
	// Perform backup
	$backupcommand = "mysqldump -u$username -p$password $database | gzip >$backupto/$backupas";
	passthru ("$backupcommand", $error);
	if($error) 
	{
	   $strMessage = "Problem: $error\n"; 
	   exit;
	}
	else
	{
		$strMessage = "$backupas successfully saved to $backupto";
	}	
}
                    ...
                    <tr> 
                      <td height="563"><!-- InstanceBeginEditable name="BODY" --> 
                        <table width="99%" height="165" border="0" cellpadding="0" cellspacing="0" bgcolor="#C1E2FF" id="BODY">
                          <tr> 
                            <td height="24" class="header"><p>BACKUP</p></td>
                          </tr>
                          <tr> 
                            <td valign="top"><table width="90%" border="0" align="center" cellpadding="0" cellspacing="0">
                                <form action="<?php $PHP_SELF; ?>" method="get">
								  <tr><td height="30" valign="middle" class="title"> Click the backup button to backup the database
                                  </td></tr>
                                <tr><td height="20" valign="middle" class="note"> 
                                    <?
								  echo $strMessage;
								  ?>
                                  </td></tr>
								  <tr><td height="50"></td></tr>
								  <tr><td height="30" valign="bottom" class="paragraph">
								  <div align="center">
								  <input name="btnBckup" type="submit" value="Backup">
								  </div>
