<?php 
/* 
File Name: OBrequest.php (class folder)
----------------------------------------------------------------------
...
class OBrequest extends Attendance
{
   ...
   function obRequest() 
   {
	...
	function viewApprovedOBRequest($strEmpNmbr, $t_strRequestID, $t_strRequestCode, $t_strEmpNumber, $t_strSurname, $t_strFirstname, $t_strMiddlename, $t_strRequestDetails, $t_dtmStatusDate, $t_strRemarks) //View list of employee/s request (OBrequest.php)
    {
		 $viewResults = mysql_query("SELECT tblEmpRequest.requestID, tblEmpRequest.requestDate, tblEmpRequest.requestCode, tblEmpRequest.requestDetails, tblEmpRequest.requestStatus, tblEmpRequest.statusDate, tblEmpRequest.remarks, tblEmpPersonal.empNumber, tblEmpPersonal.surname, tblEmpPersonal.firstname, tblEmpPersonal.middlename FROM tblEmpRequest INNER JOIN tblEmpPersonal ON tblEmpRequest.empNumber=tblEmpPersonal.empNumber WHERE tblEmpRequest.requestCode LIKE 'OB' AND tblEmpRequest.requestStatus LIKE 'Approved'");
...
//				echo "<br><br>";
				...
				echo "<td><a  href=\"OBrequest.php?strEmpNmbr=$strEmpNmbr&t_dtmRequestDate=$t_dtmRequestDate&t_strEmpNumber=$t_strEmpNumber&t_strRequestID=$t_strRequestID&t_strRequestCode=$t_strRequestCode&t_strFirstname=$t_strFirstname&t_strSurname=$t_strSurname&t_strOfficialBusiness=$t_strOBRequestDetails[0]&t_dtmDateFrom=$t_strOBRequestDetails[1]&t_dtmDateTo=$t_strOBRequestDetails[2]&t_dtmTimeFrom=$t_strOBRequestDetails[3]&t_dtmTimeTo=$t_strOBRequestDetails[4]&t_strOBPlace=$t_strOBRequestDetails[5]&t_strOBPurpose=$t_strOBRequestDetails[6]&t_dtmStatusDate=$t_dtmStatusDate&t_strRequestStatus=$t_strRequestStatus&t_strRemarks=$t_strRemarks&Submit=View\">View Details</a></td>";
				echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>";
	...
	function certifyOBRequest($t_strEmpNumber, $t_dtmOBDateFrom, $t_dtmOBDateTo, $t_strRequestID, $t_strRequestStatus, $t_strRemarks, $t_strSignatory, $Submit, $t_blnRemove=0)   // to certify ob request from approved chief module (OBrequest.php)
	{
		if ($Submit == 'Certify')
		{ 
			$intMonth = date("m", strtotime($t_dtmOBDateFrom));
			$intYear = date("Y", strtotime($t_dtmOBDateFrom));
			$intDayFrom = date("d", strtotime($t_dtmOBDateFrom));
			$intDayTo = date("d", strtotime($t_dtmOBDateTo));
			...
			$this->trapCmpltTime($t_strEmpNumber, $intMonth, $intYear, $arrDate, "OB", 0);
   ...
   function addEmpOB($t_strEmpNumber, $t_strRequestID, $t_dtmOBDateFrom, $t_dtmOBDateTo, $t_dtmOBTimeFrom, $t_dtmOBTimeTo, $t_strOBPlace, $t_strPurpose, $t_strOfficial, $t_strApproveChief, $t_strApproveHR, $t_dtmRequestDate, $Submit)   //Add ob (tblEmpOB) that was certified by the HR
   {
      if ($Submit == 'Certify')
	  {
	     $results = "INSERT INTO tblEmpOB (empNumber, requestID, dateFiled, obDateFrom, obDateTo, obTimeFrom, obTimeTo, obPlace, purpose, official, approveChief, approveHR) VALUES ('$t_strEmpNumber', '$t_strRequestID', '$t_dtmRequestDate','$t_dtmOBDateFrom', '$t_dtmOBDateTo', '$t_dtmOBTimeFrom', '$t_dtmOBTimeTo', '$t_strOBPlace', '$t_strPurpose', '$t_strOfficial', '$t_strApproveChief', '$t_strApproveHR')";
		 mysql_query($results) or die (mysql_error());
//-------------------------- code koto! code koto! baje----------------------------------------		 
		 $intNewID = mysql_insert_id();
		 $objFileOB = new FileOB;
		 $objFileOB->checkEmpDay($intNewID, $t_strEmpNumber, $t_dtmOBDateFrom, $t_dtmOBDateTo, $t_dtmOBTimeFrom, $t_dtmOBTimeTo, $t_strOfficial);
//------------------------------- end ng code ko baje -----------------------------------------		 		 
	     if(!$results) 
	     { 
