<?php 
/* 
File Name: Examinations.php (class folder)
----------------------------------------------------------------------
...
class Examinations extends General
{
	...
	function addEmployeeExam($t_strEmpNumber, $t_strExamCode, $t_strExamMonth, $t_strExamDay, $t_strExamYear, $t_intExamRating, $t_strExamPlace, $Submit)   //Add employee exmination
   {
	...
	elseif ($Submit == "Submit"){ 
		     $t_strExamDate = $this->combineDate($t_strExamYear, $t_strExamMonth, $t_strExamDay);
			 $updateResults = "UPDATE tblEmpExam SET empNumber='$t_strEmpNumber', examCode='$t_strExamCode', examDate='$t_strExamDate', examRating='$t_intExamRating', examPlace='$t_strExamPlace' WHERE empNumber='$t_strEmpNumber'";
	   ...
	   elseif ($Submit == 'OK')
	   {
	      $delete = "DELETE FROM tblEmpExam WHERE examCode='$t_strExamCode'";   //Delete Record from Database
	...
	function viewEmployeeExam($txtSearch, $optField, $p, $t_strExamCode, $t_strExamDate, $t_intExamRating, $t_strExamPlace, $t_strEmpNumber)   //view list of employee's exmination
    {
	     $viewResults = mysql_query("SELECT tblEmpExam.empNumber, tblEmpExam.examCode,
								...
								tblEmpExam.examPlace, tblExamType.examCode, 
	     ...
	     if (!$row=mysql_fetch_array($viewResults))
		 {
		    echo "  ";
			 ...
			 echo "<tr class=\"title\"><td width=\"24%\" height=\"18\" class=\"border\">EXAM CODE</td>";
			 ...
			 do 
			 {
			 	...
			 	$t_intExamRating=$row['examRating'];
			 	$t_strExamPlace=$row['examPlace'];
				echo "<tr class=\"border\"><td class=\"border\">" . $row['examCode'] . "</td>";
