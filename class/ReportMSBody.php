...
require('../hrmis/class/ReportMonthlySeparation.php');

class ReportMSBody extends General
{
	var $objRprt;
	var $intCounter = 0;


	function printBody($t_intCounter, $t_strEmpNum ,$t_strEmpName, $t_strBirthday,$t_strPositionDesc,$t_strAppointmentDesc,$t_strSGNumber, $t_strStatusOfAppointment, $t_strPositionDate)
	 {
		...
		$this->objRprt->Cell(60,10,$t_strPositionDesc,1,0,L);         //  Position Title
		...
		$this->objRprt->Cell(30,10,$t_strStatusOfAppointment,1,0,C);  //  Mode of Separation
		$this->objRprt->Cell(30,10,$t_strPositionDate,1,0,C);         //  Effectivity Date Of Appointment
		$this->objRprt->Cell(50,10,' ',1,1,L);                      //  Remarks
	  }

	function generateReport()
	{
		$this->objRprt = new ReportMonthlySeparation;
		...
		while($arrQuery = mysql_fetch_array($objQuery))
		 {
			...
			$strContractEndDateEx = explode ('-', $strContractEndDate);
			$arrContractEndDateYr = $strContractEndDateEx[0];
			$arrContractEndDateMonth = $strContractEndDateEx[1];
			$arrContractEndDateExDay = $strContractEndDateEx[2];
			$sesYear = $_SESSION['sesYear'];
			$sesMonth = $_SESSION['sesMonth'] ;
					
			if (($strStatusOfAppointment !='In-Service')&&($arrContractEndDateYr == $sesYear) &&($arrContractEndDateMonth == $sesMonth))
			{
				
				$this->intCounter++;
				$intPageCounter++;
				$this->printBody($this->intCounter,$strEmpNum ,$strEmpName, $strBirthday,$strPositionDesc,$strAppointmentDesc,$strSGNumber, $strStatusOfAppointment, $strPositionDate);
