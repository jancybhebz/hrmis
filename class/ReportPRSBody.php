<?
/* 
File Name: ReportPRSBody.php (class folder)
----------------------------------------------------------------------
...
require_once('../hrmis/class/ReportPersonnelResigned.php');

class ReportPRSBody extends General
{
	var $objRprt;
	var $intCounter = 0;
	...
	{
		$this->objRprt = new ReportPersonnelResigned;
		...
		$objEmpDateResigned = mysql_query("SELECT tblEmpPersonal.empNumber, tblEmpPersonal.surname,
									...
									WHERE tblEmpPosition.statusOfAppointment = 'Resigned'
										OR tblEmpPosition.statusOfAppointment = 'RESIGNED'
									ORDER BY tblEmpPersonal.surname, tblEmpPersonal.firstname,
										   tblEmpPersonal.middlename asc");
		
		$intCounter = 0;
		while($arrEmpDateResigned = mysql_fetch_array($objEmpDateResigned))
		{
			$strEmpNum = $arrEmpDateResigned['empNumber'];
			$strMidName = $arrEmpDateResigned['middlename'];
			$strMiddleName = substr($strMidName,0,1);
			$strEmpName = $arrEmpDateResigned['surname']. ",  ".$arrEmpDateResigned['firstname']. " ".$strMiddleName.".";
			$strStatusOfAppointment = $arrEmpDateResigned['statusOfAppointment'];
			$dtmContractEndDate = $arrEmpDateResigned['contractEndDate'];
			$dtmDateResigned = substr($dtmContractEndDate,0,4);

			if($strStatusOfAppointment == 'Resigned' || $strStatusOfAppointment == 'RESIGNED')
			{
				$intCounter++;
